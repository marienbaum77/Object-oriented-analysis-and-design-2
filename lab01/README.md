...
Продуктом выступает абстрактный класс **`DrumKit`**. В отличие от простой загрузки данных, здесь фабричный метод позволяет возвращать объекты с различным внутренним поведением. Клиентский код (**`MainWindow`**) полностью изолирован от знаний о файловой системе и специфических аудио-эффектах. Он оперирует списком интерфейсов `IKitFactory` и при смене жанра просто вызывает метод `CreateKit()`, получая готовый объект с нужной логикой внутри.
