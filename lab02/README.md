...
## 4. Вывод С паттерном добавление нового элемента (например, TodoListItem или ImageItem) требует только создания нового класса. Старый код (статистика, сохранение, UI) вообще не меняется. Без паттерна нужно будет переписывать каждый метод в контроллере и QML.
