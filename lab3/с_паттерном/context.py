...
class Portfolio:
    ...
    def apply_buy(self, ticker: str, qty: int, total_cost_rub: float) -> None:
        with self._lock:
            ...
            if new_qty <= 0:
                return
            # Weighted average buy price (per share); total_cost includes commission.
...
def default_state_path() -> str:
    return os.environ.get(
        "MOEXCHAIN_STATE_PATH",
        os.path.join(os.getcwd(), "moex_chain_trader_state.json"),
    )
...
@dataclass
class TradeContext:
    """Mutable context passed sequentially through the handler pipeline."""

    event: str  # "tick" | "buy" | "sell" | "poll"
    portfolio: Portfolio
    ...
    # (unrealized_rub, pnl_percent)
    status_messages: List[Tuple[str, str]] = field(default_factory=list)
    last_handler: Optional[str] = None
    stop_chain: bool = False
    stop_reason: Optional[str] = None
    network_error: Optional[str] = None  # last background fetch issue (non-fatal if cache exists)

    def log(self, handler_name: str, message: str) -> None:
        self.status_messages.append((handler_name, message))
        self.last_handler = handler_name
